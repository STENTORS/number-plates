...
def year(licence):
    ...
    if len(count) == 6:
        ...
        if alphabet(0,2) == 1 and alphabet(2,3) == 0:
            ...
            guiData.append(diplomatOne)
            guiData.append(diplomatTwo)

            if diplomatTwo >= 101 and diplomatTwo <= 399:
                guiData.append("Diplomat acording to the 101 - 399")
            elif diplomatTwo >= 400 and diplomatTwo <= 699:
                guiData.append("non-diplomatic staff of international organisations")
            elif diplomatTwo >= 700:
                guiData.append("consular or other non-diplomatic staff")
                
            locationDiplomatPrint = diplomatLocation(diplomatOne)
            guiData.append(locationDiplomatPrint)
            
            if count[3] =="D" or count[3] == "d":
                guiData.append("diplomatic staff within london?")
            elif count[3] =="X" or count[3] == "x":
                guiData.append("Non-diplomatic accredited personnel / outside of London?")
...
def runLicense(inVal):
    while True:
        ...
        if len(inVal) <= 8:
            ...
            guiData.append("Invalid Licence or possibly a custom one")
    ...
    if (currentSystem):
        ...
        if yearOne >= 5:
            yearOne = yearOne - 5
            guiData.append("Second half of:")
        else:
            guiData.append("First half of:")
        yearOne = str(yearOne)
            
        yearOut = "20"+yearOne+yearTwo
        guiData.append(yearOut)

        #Local memory tag check
        location = count[0] + count[1]
        region, office = licenseLocation(location)
        guiData.append(region)
        guiData.append(office)
